...
// import { submitClosure } from "../services/closureApi";

import { setClosureType } from "../store/closureSlice";
...
export default function ClosurePage() {
  ...
  const { selectedClosureType } = useAppSelector((state) => state.closures);

  const [proofs, setProofs] = useState<ClosureProof[]>([]);

  const [remarks, setRemarks] = useState("");

  const [submittedClosure, setSubmittedClosure] =
    useState<ClosureRecord | null>(null);

  const [submitting, setSubmitting] = useState(false);

  const [visitData, setVisitData] = useState<VisitClosureData | undefined>();

  const [partData, setPartData] = useState<PartClosureData | undefined>();
  ...
  const [uninstallationData, setUninstallationData] = useState<
  ...
  if (submittedClosure) {
    return (
      <div className="space-y-6">
        <ClosureSummary closure={submittedClosure} />
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={() => navigate("/closures/history")}
  ...
  const submitCurrentForm = () => {
    const forms = {
      VISIT: "visit-closure-form",

      PART: "part-closure-form",

      SERVICE: "service-closure-form",

      INSTALLATION: "installation-closure-form",

      UNINSTALLATION: "uninstallation-closure-form",
    };

    if (!selectedClosureType) {
      alert("Please select a closure type");

      return;
    }

    document.getElementById(forms[selectedClosureType])?.dispatchEvent(
      new Event("submit", {
        bubbles: true,
        cancelable: true,
      }),
  ...
  const finalizeClosure = async (
    ...
    try {
      ...
      switch (selectedClosureType) {
        case "VISIT":
          payload.visitData = typeData as VisitClosureData;
          break;

        case "PART":
          payload.partData = typeData as PartClosureData;
          break;

        case "SERVICE":
          payload.serviceData = typeData as ServiceClosureData;
          break;

        case "INSTALLATION":
          payload.installationData = typeData as InstallationClosureData;
          break;

        case "UNINSTALLATION":
          payload.uninstallationData = typeData as UninstallationClosureData;
          break;
      }

      const closure = await submitClosure(payload);

      setSubmittedClosure(closure);
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Complete closure details for <strong>{complaintId}</strong>.
          </p>
        </div>
      ...
      <Card className="p-6">
        ...
        <div className="mt-5">
          <ClosureTypeSelector
            ...
            onChange={(type) => dispatch(setClosureType(type))}
          />
        </div>
      </Card>
      ...
      {selectedClosureType && (
        <Card className="p-6">
          ...
          {selectedClosureType === "VISIT" && (
            <VisitClosureForm
              onSubmit={(data) => {
                setVisitData(data);

                finalizeClosure(data);
              }}
            />
          )}

          {selectedClosureType === "PART" && (
            <PartClosureForm
              onSubmit={(data) => {
                setPartData(data);
          ...
          {selectedClosureType === "INSTALLATION" && (
          ...
          {selectedClosureType === "UNINSTALLATION" && (
            <UninstallationClosureForm
              onSubmit={(data) => {
                setUninstallationData(data);
      ...
      {selectedClosureType && (
        <>
          <Card className="p-6">
            <ProofUploader proofs={proofs} onChange={setProofs} />
          </Card>
