...
export default function RoleListPage() {
  ...
  const { search, status } = useAppSelector((state) => state.accessControl);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Control what each application role can access.
          </p>
        </div>

       {hasPermission("roles.create") && <button
      ...
      <div className="flex flex-col gap-3 rounded-xl border bg-white p-4 md:flex-row">
        ...
        <button
          onClick={() => dispatch(clearRoleFilters())}
...
{    hasPermission("roles.view") && (loading ? (
        <div className="rounded-xl border bg-white p-12 text-center">
          Loading roles...
        </div>
      ) : (
        <RoleTable roles={filteredRoles} onDelete={deleteRole} />
      ))}
    </div>
  );
}
